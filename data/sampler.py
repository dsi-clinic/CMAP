...
class ClassBalancedRandomBatchGeoSampler(BatchGeoSampler):
    ...
    def __init__(self, config) -> None:
        ...
        self.batch_size = config["batch_size"]
        self.length = 0
        self.hits, self.areas, self.class_areas = (
            self.calculate_hits_and_areas()
        )  # Added self.class_areas to store the per‐hit class area distributions
    ...
    def calculate_hits_and_areas(
        ...
        hits = []
        areas = []
        class_areas = []  # Adding a new list to keep track of class areas
        context_x = self.size[1] / 2
        context_y = self.size[0] / 2
        ...
        for hit in self.index.intersection(tuple(self.roi), objects=True):
            ...
            if (
                ...
                # Calling a new helper function compute_class_area()
                per_class = {}  # dict: {class_id: area}
                for class_id in config.KC_LABELS:  # KC_LABELS is defined in the config.py file, still need to find a way to code this to properly reference that dictionary
                    # You may need to implement compute_class_area() using shapely or similar
                    per_class[class_id] = compute_class_area(
                        hit, class_id, shape_bounds
                    )  # new helper function compute_class_area()
                class_areas.append(per_class)

        areas_tensor = torch.tensor(areas, dtype=torch.float)
        if torch.sum(areas_tensor) == 0:
            areas_tensor += 1

        return hits, areas_tensor, class_areas
    ...
    def __iter__(self):
        ...
        for _ in range(len(self)):
            batch = []
            batch_class_info = []  # list to keep track of the per batch class information
            for _ in range(self.batch_size):
                idx = torch.multinomial(self.areas, 1)
                hit = self.hits[idx]
                class_info = self.class_areas[
                    idx
                ]  # References the list crated in line 338. Might be better to output a dictionary with {class:area}
                bounds = BoundingBox(*hit.bounds)
                bounding_box = get_random_bounding_box(bounds, self.size, self.res)
                batch.append(bounding_box)
                batch_class_info.append(class_info)
            yield {
                "batch": batch,
                "class_info": batch_class_info,
            }  # Yielding the bach and the class info (dont know if this will break the dataloader's collate_fn when it calls the sampler)
