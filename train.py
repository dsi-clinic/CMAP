...
from torchgeo.datasets import NAIP, random_bbox_assignment, stack_samples, IntersectionDataset, GeoDataset, BoundingBox
...
train_dataset = (train_portion & kc) & dem
test_dataset = (test_portion & kc) & dem

# Create a DataLoader for iterating over the dataset
dataloader = DataLoader(train_dataset, batch_size=1, shuffle=False)

def get_random_bounding_box(dataset):
    # Get the spatial extent of the dataset
    minx, maxx, miny, maxy, mint, maxt = dataset.bounds

    # Generate random coordinates within the spatial extent
    ...
    random_mint = random.uniform(mint, maxt)
    random_maxt = random.uniform(random_mint, maxt)

    # Construct the random BoundingBox object
    random_bbox = BoundingBox(random_minx, random_maxx, random_miny, random_maxy, random_mint, random_maxt)

    return random_bbox

random_bbox = get_random_bounding_box(train_dataset)

# for batch, sample in enumerate(dataloader):
#     samp_image = sample["image"]
#     num_bands = samp_image.size(1)
#     break

sample = train_dataset[random_bbox]
samp_image = sample["image"]
num_bands = samp_image.size(1)
