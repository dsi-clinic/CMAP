...
if __name__ == "__main__":
    ...
    def run_trials():
        ...
        # Only set these parameters if they exist in wandb.config
            # config.COLOR_BRIGHTNESS = wandb.config.get("COLOR_BRIGHTNESS", config.COLOR_BRIGHTNESS)
            # config.GAUSSIAN_BLUR_SIGMA = wandb.config.get("GAUSSIAN_BLUR_SIGMA", config.GAUSSIAN_BLUR_SIGMA)
            # config.GAUSSIAN_BLUR_KERNEL = wandb.config.get("GAUSSIAN_BLUR_KERNEL", config.GAUSSIAN_BLUR_KERNEL)
            config.LR = wandb.config.get("LEARNING_RATE", config.LR)
            # config.ATTENTION_HEAD_DIM = wandb.config.get("ATTENTION_HEAD_DIM", config.ATTENTION_HEAD_DIM)
            # config.LAYERS_PER_BLOCK = wandb.config.get("LAYERS_PER_BLOCK", config.LAYERS_PER_BLOCK)
            # config.NORM_NUM_GROUPS = wandb.config.get("NORM_NUM_GROUPS", config.NORM_NUM_GROUPS)
            # config.NORM_EPS = wandb.config.get("NORM_EPS", config.NORM_EPS)
        train_ious = []
        test_ious = []
