...
if __name__ == "__main__":
    ...
    with rasterio.open(filled_path) as filled_src, rasterio.open(dem_path) as normal_src:
        profile = filled_src.profile
        profile.update(dtype=rasterio.float32, nodata=-9999)

        with rasterio.open(output_path, "w", **profile) as dst:
            for _, window in filled_src.block_windows(1):  # Unpack tuple (block_id, window)
                filled_dem = filled_src.read(1, window=window)
                normal_dem = normal_src.read(1, window=window)

                dem_diff = np.where(filled_dem != normal_dem, filled_dem - normal_dem, 0)
