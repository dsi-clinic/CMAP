...
"""
KC_images_data = kc.KaneCounty(root="/net/projects/cmap/data/KC-images") # check if we need to define this root, or just leave it as root
KC_masks_data = kc.KaneCounty(root="/net/projects/cmap/dataKC-masks/top-structures-masks")
...
test_data_len = 50
dataset = torch.utils.data.Subset(img_masks_data, indices[:-test_data_len])
dataset_test = torch.utils.data.Subset(img_masks_data, indices[-test_data_len:])
...
# change to grid sampling for test data
...
# Model
model_backbone = "resnet50" # test changes in backbone
weights = "imagenet" # test changes in weights, can be None
in_channels = 1 # Number of input channels for the model, default is 3 for RGB images
num_classes = 4 # Number of prediction classes

# Unet model
model = smp.Unet(encoder_name=model_backbone, 
                 encoder_weights=weights if weights is True else None,
...
# Loss function
loss_fn = smp.losses.JaccardLoss(mode="multiclass", classes=num_classes) 
