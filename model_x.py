...
class ModifiedUNet2DConditionModel(nn.Module):
    def __init__(self, unet_model, num_classes):
        super(ModifiedUNet2DConditionModel, self).__init__()
        self.unet = unet_model
        self.segmentation_head = EnhancedSegmentationHead(self.unet.conv_out.out_channels, num_classes)

    def forward(self, x, timesteps=None, context=None):
        # Forward pass through the UNet
        unet_output = self.unet(x, timesteps, context)
        
        # Forward pass through the enhanced segmentation head
        segmentation_output = self.segmentation_head(unet_output.sample)
    ...
    def __init__(self, model_config):
        ...
        if model != "fcn":
            ...
            if model == "diffsat":
                ...
                self.model.conv_out = EnhancedSegmentationHead(self.model.conv_out.in_channels, self.num_classes)
